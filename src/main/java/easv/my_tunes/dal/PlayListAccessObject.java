...
public class PlayListAccessObject {
    ...
    public List<Playlist> getPlaylists(){
        ...
        try (Connection con = ConnectionManager.getConnection()) {
            String sqlPrompt = "select playlists.id as playlist_id, playlists.name as playlist_name, playlist_songs.song_id as song_id, songs.title as song_title, songs.artist as song_artist, songs.category as song_category, songs.time_in_seconds as song_time, songs.path as song_path from playlists LEFT JOIN playlist_songs ON playlists.id = playlist_songs.playlist_id LEFT JOIN songs on playlist_songs.song_id = songs.id";
